...
with app:
    gr.Markdown("# 🧪 Chemical Experiment Optimization Interface (BoTorch - Multi-Objective)")
    ...
    with gr.Tab("Suggest & Save Results 💡"):
        ...
        def update_pmi_ui(base, base_eq, lag, eta, yield_val):
            """UI wrapper to calculate PMI and return the value."""
        ...
        with suggestions_ui_container:
                ...
                with gr.Group(visible=(exp_idx_ui == 0)) as exp_group_ui: 
                    ...
                    for i, col_name_param_ui in enumerate(ORDERED_INPUT_COLS_FOR_UI):
                        ...
                        if var_spec_param_ui['type'] == 'categorical':
                            ...
                            precision_val_ui = 0
                            if col_name_param_ui == 'eta': precision_val_ui = 2
                            elif col_name_param_ui == 'freq': precision_val_ui = 1 
                            elif col_name_param_ui != 'time': precision_val_ui = 2 
            ...
            for exp_set in experiment_suggestion_sets_ui:
                ...
                lag_comp = exp_set['input_params']['lag']
                eta_comp = exp_set['input_params']['eta']

                yield_comp.change(
                    fn=update_pmi_ui,
                    inputs=[base_comp, base_eq_comp, lag_comp, eta_comp, yield_comp],
                    outputs=[pmi_comp]
                )
