...
def _format_suggestion_for_ui(suggestion_series):
    ...
    for col_name in ORDERED_INPUT_COLS_FOR_UI:
        ...
        if var_info['type'] == 'categorical':
            ...
            val_float = float(val)
            if col_name == "freq":
                f_bounds = var_info['bounds']
                val_clamped = min(max(val_float, f_bounds[0]), f_bounds[1])
                ui_outputs.append(round(val_clamped * 2) / 2)
            elif col_name == "time":
                ui_outputs.append(round(val_float))
            elif col_name == "eta" or col_name == "base_eq":
                 ui_outputs.append(round(val_float, 2)) 
            else: 
                ui_outputs.append(round(val_float, 2)) 
    return ui_outputs
