...
def calculate_pmi(s_salt, base, base_eq, solvent, eta, yield_):
    """
    Calculate Process Mass Intensity (PMI) for Aldehyde + S-Salt -> Epoxide.
    Assumptions:
      - Aldehyde: 1 eq (fixed scale).
      - S-Salt: 1 eq (TMSI or TMSOI).
      - Base: variable (1–3 eq).
      - Solvent mass approximated: eta * density * (reagents + base).
    ...
    try:
        ...
        if s_salt is None or s_salt not in PMI_CONFIG['reagents']['s_salt']:
            return None, f"S-Salt '{s_salt}' invalid. Choose from {list(PMI_CONFIG['reagents']['s_salt'].keys())}."
        if base is None or base not in PMI_CONFIG['bases']:
            return None, f"Base '{base}' invalid."
        if solvent is None or solvent not in PMI_CONFIG['solvents']:
            return None, f"Solvent (LAG) '{solvent}' invalid."
        if base_eq is None or str(base_eq).strip() == "":
            return None, "Base equivalents are required."
        beq = float(base_eq)
        ...
        mass_aldehyde = PMI_CONFIG['reagents']['aldehyde']['mass_mg']
        ...
        mass_reagents = mass_aldehyde + mass_salt + mass_base
...
def load_data():
    """Load data, migrate legacy column names, ensure required columns."""
...
def save_suggestion_result(*args):
    ...
    # special eta==0 replication updated (solvent)
